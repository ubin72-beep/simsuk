...
function startAutoRefresh() {
    ...
    // 30초마다 주문 확인 (GitHub Pages - 데모 환경에서는 작동 안 함)
    autoRefreshInterval = setInterval(async () => {
        if (!autoRefreshEnabled) return;
        
        console.log('🔄 [Notifications] 자동 새로고침 체크 (데모 환경 - 실제 업데이트 없음)');
        
        // GitHub Pages 환경에서는 실제 새 주문이 없으므로 
        // adminOrders 배열의 변화만 감지
        const currentOrderCount = (window.adminOrders || []).length;
        
        // 새 주문 감지 (실제로는 발생하지 않음)
...
// 초기 주문 수 설정 (GitHub Pages - 데모 데이터)
async function initializeAutoRefresh() {
    console.log('🔔 [Notifications] 자동 새로고침 초기화...');
    
    // adminOrders 배열의 현재 크기로 초기화
    lastOrderCount = (window.adminOrders || []).length;
    console.log(`✅ [Notifications] 초기 주문 수: ${lastOrderCount}개`);
